...
if (is_array($_merchantProfileId)){
	foreach($_merchantProfileId as $profileId){
		...
		if (is_array($_bankcardServices->BankcardService) && $_bankcardServices->BankcardService[0] instanceof BankcardService){
		...
		{
			$response = $client->authorize($bcpTxn->TndrData, $bcpTxn->TxnData, true);
		...
		{
			$response = $client->authorize($bcpTxn->TndrData, $bcpTxn->TxnData, $bcpTxn->TxnData->Creds);
		...
		if($_bcs->Operations->Undo)
		{
			...
			$response3 = $client->authorize($bcpTxn->TndrData, $bcpTxn->TxnData, $bcpTxn->TxnData->Creds);
		...
		{
			$response5 = $client->authorizeAndCapture($bcpTxn->TndrData, $bcpTxn->TxnData, $bcpTxn->TxnData->Creds);
