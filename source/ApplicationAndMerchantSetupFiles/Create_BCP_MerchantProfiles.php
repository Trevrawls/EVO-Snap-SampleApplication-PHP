...
if ($_merchantProfileId == null){
	...
	if (isset($_serviceId[1])){
		...
		foreach ($_serviceId as $svcId){
			...
			if ($svcId['ServiceId'] == '4365400001' || $svcId['ServiceId'] == '6781E00001' || $svcId['ServiceId'] == 'D806000001'){// Vantiv
...
if(is_array($response) && isset($response[0])  
	&& isset($_serviceId['singleService']['ServiceId']) 
	&& $_msgFormat == 'JSON')
{		
	$response = $client->getMerchantProfile($_serviceId['singleService']['ServiceId'], $response[0]->id);
}
...
function setBCPMerchantProfile(){
	...
	// <!--type: string--> Optional - Required for FTPS and TSYS
	$bankCardMerchData->AgentChain = '1289'; // Was 128965
	// <!--type: string--> Optional - Required for FTPS and TSYS
	...
	// <!--type: string--> Optional - Required for Chase
